...
const pinoLogger = pino(fs.createWriteStream("/dev/null")).child({
  module: "test",
});
const pinoExtreme = pino(
  pino.destination({ sync: false, dest: "/dev/null" }),
).child({ module: "test" });
...
export default function childLogging() {
